...
function convertMessageToCohereMessage(message, toolResults) {
	...
	const getToolCall = (message) => {
		if ((0, _langchain_core_messages.isAIMessage)(message) && message.tool_calls) return message.tool_calls.map((toolCall) => ({
			name: toolCall.name,
...
function _formatToolsToCohere(tools) {
	...
	else if (tools.every(_langchain_core_language_models_base.isOpenAITool)) return tools.map((tool) => {
		return {
	...
	else if (tools.every(_langchain_core_utils_function_calling.isLangChainTool)) return tools.map((tool) => {
		const parameterDefinitionsFromZod = (0, _langchain_core_utils_types.isInteropZodSchema)(tool.schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(tool.schema) : tool.schema;
		return {
			name: tool.name,
...
* <br />
*/
var ChatCohere = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
		return "ChatCohere";
	}
	...
	constructor(modelOrFields, fields) {
		...
		super(params);
		this._addVersion("@langchain/cohere", "1.0.4");
		this.client = require_client.getCohereClient(params);
	...
	_getChatRequest(messages, options) {
		...
		if (!params.forceSingleStep) {
			...
			for (let i = 0; i < messages.length - 1; i += 1) {
				const message = messages[i];
				if ((0, _langchain_core_messages.isAIMessage)(message) && message.tool_calls) continue;
				if (message._getType().toLowerCase() === "tool") {
					tempToolResults = tempToolResults.concat(this._messageToCohereToolResults(messages, i));
					if (i === messages.length - 1 || !(messages[i + 1]._getType().toLowerCase() === "tool")) {
	...
	_messagesToCohereToolResultsCurrChatTurn(messages) {
		...
		for (const message of currChatTurnMessages) if (isToolMessage(message)) {
			...
			const previousAiMsgs = currChatTurnMessages.filter((msg) => (0, _langchain_core_messages.isAIMessage)(msg) && msg.tool_calls !== void 0);
	...
	_messageToCohereToolResults(messages, toolMessageIndex) {
		...
		const previousAiMessage = messages.slice(0, toolMessageIndex).filter((message) => (0, _langchain_core_messages.isAIMessage)(message) && message.tool_calls).slice(-1)[0];
		...
		for await (const chunk of stream) {
			...
			if (chunk.eventType === "text-generation") {
				yield new _langchain_core_outputs.ChatGenerationChunk({
					text: chunk.text,
					message: new _langchain_core_messages.AIMessageChunk({ content: chunk.text })
				});
				await runManager?.handleLLMNewToken(chunk.text);
			} else if (chunk.eventType !== "stream-end") yield new _langchain_core_outputs.ChatGenerationChunk({
				text: "",
