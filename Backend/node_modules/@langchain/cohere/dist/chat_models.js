...
function convertMessageToCohereMessage(message, toolResults) {
	...
	const getToolCall = (message) => {
		if (isAIMessage(message) && message.tool_calls) return message.tool_calls.map((toolCall) => ({
			name: toolCall.name,
...
var ChatCohere = class extends BaseChatModel {
	...
	_messageToCohereToolResults(messages, toolMessageIndex) {
		...
		const previousAiMessage = messages.slice(0, toolMessageIndex).filter((message) => isAIMessage(message) && message.tool_calls).slice(-1)[0];
