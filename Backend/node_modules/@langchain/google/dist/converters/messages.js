...
function convertStandardContentMessageToGeminiContent(message, messages) {
	...
	else if (ToolMessage.isInstance(message)) role = "user";
	...
	if (AIMessage.isInstance(message) && message.tool_calls?.length) for (const toolCall of message.tool_calls) parts.push({ functionCall: {
		name: toolCall.name,
		args: toolCall.args ?? {}
	} });
	if (ToolMessage.isInstance(message) && message.tool_call_id) {
		...
		const matchedToolCall = messages.filter(AIMessage.isInstance).find((msg) => msg.tool_calls?.find((tc) => tc.id === message.tool_call_id))?.tool_calls?.find((tc) => tc.id === message.tool_call_id);
...
function convertLegacyContentMessageToGeminiContent(message, messages) {
	...
	*/
	...
	const role = iife(() => {
		...
		else if (ToolMessage.isInstance(message)) return "user";
		else if (ChatMessage.isInstance(message)) {
			const msgRole = message.role.toLowerCase();
...
const convertGeminiPartsToToolCalls = (parts) => {
	...
	for (let i = 0; i < parts.length; i++) {
		...
		if ("functionCall" in part && part.functionCall) {
			...
			toolCalls.push({
				type: "tool_call",
				id: functionCallPart.functionCall.id ?? `lc-tool-call-${v4().replace(/-/g, "")}`,
